...
`packaging/recipes/stackpro/all/conanfile.py`). Scoped to what isn't obvious
...
- **The release tarball extracts to `StackAllocator-<version>`, not
  `StackPro-<version>`** — the GitHub repo is `StackAllocator`; the CMake
  project/library target inside it is `StackPro`. `cd` into whatever
...
```bash
find $HOME/staging/lib/cmake/StackPro -name "*.cmake"
```

Expect `StackProConfig.cmake`, `StackProConfigVersion.cmake`,
`StackProTargets.cmake` (plus a per-config
`StackProTargets-<config>.cmake`) under `lib/cmake/StackPro/`.
...
target_link_libraries(test PRIVATE StackPro::StackPro)
EOF

cat > main.cpp << 'EOF'
#include <StackPro/Stack.h>
int main() {
    rain::Stack<> stack(1024);
    return 0;
}
EOF
...
just constructs a `rain::Stack` and returns, so success already
confirms the header parsed, the library linked, and the symbols
resolved.
...
```bash
conan create packaging/recipes/stackpro/all --version 1.0.0 \
...
`packaging/recipes/stackpro/all/test_package/` against the freshly-built
package — a `StackPro linked and constructed successfully.` line in
