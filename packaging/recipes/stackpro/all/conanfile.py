...
class Conan(ConanFile):
    ...
    # edited here.
    name = "stackpro"
    cmake_name = "StackPro"  # matches project()'s name in the top-level CMakeLists.txt
    version = "1.0.0"

    url = "https://github.com/privateMwb/StackAllocator"
    description = "High-performance, header-only linear (stack) allocator with O(1) allocate/reset/rollback, backed by a single bump-pointer buffer with marker-based checkpoints."
    topics = (
        "allocator",
        "memory-allocator",
        "stack-allocator",
    ...
    # header-library, not "library": there's no compiled src/StackPro/*.cpp
