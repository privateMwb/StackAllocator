 ...
 *
 * Contains the implementation of Stack template member functions and
 * internal implementation details.
 */

// ===========================================================
// Template implementation for StackPro::Stack.
//
//  Sections:
...
Stack<EnableStats>::Stack(std::size_t size, std::size_t alignment)
    : memory_{allocateMemory(size, alignment)}, cap_{size}, offset_{0},
      alignShift_{toShift(alignment)}, stats_{} {}

template <bool EnableStats> Stack<EnableStats>::~Stack() {
...
Stack<EnableStats>::Stack(Stack&& other) noexcept
    : memory_{std::exchange(other.memory_, nullptr)}, cap_{std::exchange(other.cap_, 0)},
      ...
      // alignShift_ is not reset: it's meaningless once memory_ is null, and
      // zeroing it would make a moved-from stack's default-alignment
      // allocate() calls trip the alignment precondition instead of simply
      // failing the (already zeroed) capacity check.
      ,
...
std::byte* Stack<EnableStats>::allocate(std::size_t size, std::size_t requestAlignment) noexcept {
    ...
    AP_PRE(requestAlignment <= (std::size_t{1} << alignShift_));

    // Compute the aligned allocation offset.
    const std::size_t alignedOffset = alignForward(offset_, toShift(requestAlignment));

    // Check available capacity. Subtraction-based (rather than
    // alignedOffset + size > cap_) to avoid overflow when size is
    // close to SIZE_MAX; alignedOffset <= cap_ is an invariant here,
    // since alignForward() only ever grows offset_ within [0, cap_].
    if (alignedOffset > cap_ || size > cap_ - alignedOffset)
        return nullptr;
...
template <bool EnableStats> auto Stack<EnableStats>::getMarker() const noexcept -> Marker {
    return Marker{offset_};
}

template <bool EnableStats> void Stack<EnableStats>::freeToMarker(Marker marker) noexcept {
...
template <bool EnableStats> void Stack<EnableStats>::reset() noexcept {
    offset_ = 0;
...
template <bool EnableStats> bool Stack<EnableStats>::owns(const void* ptr) const noexcept {
    const auto* p = static_cast<const std::byte*>(ptr);

    // std::less/std::less_equal give a well-defined total order even
    // for pointers that aren't part of the same array (unlike raw
    // <, <=, which are only defined within a common array object).
    return !std::less<const std::byte*>{}(p, memory_) &&
           std::less<const std::byte*>{}(p, memory_ + cap_);
}
...
template <bool EnableStats> std::size_t Stack<EnableStats>::used() const noexcept {
    return offset_;
}

template <bool EnableStats> std::size_t Stack<EnableStats>::remaining() const noexcept {
    return cap_ - offset_;
}

template <bool EnableStats> std::size_t Stack<EnableStats>::capacity() const noexcept {
    return cap_;
}
...
template <bool EnableStats>
constexpr std::size_t Stack<EnableStats>::alignForward(std::size_t ptr,
...
template <bool EnableStats> constexpr void Stack<EnableStats>::statDealloc() noexcept {
    if constexpr (EnableStats)
