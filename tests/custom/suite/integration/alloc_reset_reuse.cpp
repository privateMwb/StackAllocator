...
static void fill_then_overflow_fails() {
    Stack<> stack(32);
...
static void reset_reclaims_capacity() {
    Stack<> stack(32);
...
static void reused_addr_after_reset() {
    Stack<> stack(32);
...
static void run_tests() {
    RUN(fill_then_overflow_fails);
    RUN(reset_reclaims_capacity);
    RUN(reused_addr_after_reset);
}
