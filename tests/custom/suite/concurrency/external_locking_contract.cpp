...
static void used_matches_alloc_count() {
...
static void free_to_marker_locked() {
    Stack<> stack(1024);
    std::mutex lock;
...
static void run_tests() {
    RUN(allocs_no_overlap);
    RUN(used_matches_alloc_count);
    RUN(free_to_marker_locked);
}
