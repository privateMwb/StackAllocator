...
static void rejects_overflow_size() {
    Stack<> stack(64);
...
static void guard_holds_partial_alloc() {
    Stack<> stack(64);
...
static void exact_remaining_succeeds() {
    Stack<> stack(32);
...
static void run_tests() {
    RUN(rejects_overflow_size);
    RUN(guard_holds_partial_alloc);
    RUN(exact_remaining_succeeds);
}
