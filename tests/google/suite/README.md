...
- **Registration** — every case is a `TEST(<Suite>, <TestName>)`. The suite is
  the CamelCase name of the API or behavior area under test (`Allocate`,
  `GetMarker`, `ScopeRollback`, `ExternalLockingContract`), and the test name
  is a CamelCase description of the behavior
  (`ReturnsNullptrWhenOutOfSpace`) — no underscores. Google Test registers
  each `TEST` itself, so there is no `run_tests()`, no
  `REGISTER_TEST_SUITE()`, and no `main` in these files; link against
  `gtest_main` or supply your own. Run one suite with
  `--gtest_filter=Allocate.*`.
- **Assertions** — `EXPECT_*` by default, so a failing check doesn't stop the
  rest of the case: `EXPECT_EQ(actual, expected)` for values (unsigned
  literals such as `0u` against size types), `EXPECT_NE`/`EXPECT_EQ` against
  `nullptr` for pointers, `EXPECT_TRUE`/`EXPECT_FALSE` for booleans, and
  `EXPECT_THROW` for an expected exception. `ASSERT_NE(p, nullptr)` is used
  only where a null pointer would be dereferenced or offset on the next line.
  A case whose only check is that nothing crashes ends in `SUCCEED()`.
- **Structure** — each file includes `<StackPro/Stack.h>` and
  `<StackPro/StackScope.h>`, then `<gtest/gtest.h>`. Each opens with a header
  comment naming its subject and a `Coverage:` bullet list, and each `TEST`
  has a `// Verifies ...` comment above it.
...
- **Concurrency** — worker threads only set atomic flags or write to their
  own slot; every `EXPECT_*` runs on the main thread after `join()`, since
