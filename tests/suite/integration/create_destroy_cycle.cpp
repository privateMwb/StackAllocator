...
static void destructor_runs_exactly_once() {
...
static void destroy_then_create_does_not_reuse_slot() {
    Stack<> stack(64);
...
static void free_to_marker_after_destroy_reuses_memory() {
    Stack<> stack(64);
...
static void run_tests() {
    RUN(destructor_runs_exactly_once);
    RUN(destroy_then_create_does_not_reuse_slot);
    RUN(free_to_marker_after_destroy_reuses_memory);
}
