...
static void inner_scope_leaves_outer_allocations_intact() {
    Stack<> stack(64);
...
static void nested_scopes_unwind_in_lifo_order() {
    Stack<> stack(64);
...
static void sibling_scopes_return_to_same_offset() {
    Stack<> stack(64);
    const std::size_t start = stack.used();
...
static void run_tests() {
    RUN(inner_scope_leaves_outer_allocations_intact);
    RUN(nested_scopes_unwind_in_lifo_order);
    RUN(sibling_scopes_return_to_same_offset);
}
